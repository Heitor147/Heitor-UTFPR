...
INSERT INTO `vendas` VALUES (1,54215,'2024-06-25',40,1,1,1,1,1,'2024-06-25 07:52:21');
/*!40000 ALTER TABLE `vendas` ENABLE KEYS */;
UNLOCK TABLES;

--
-- Temporary view structure for view `vendas_detalhadas`
--

DROP TABLE IF EXISTS `vendas_detalhadas`;
/*!50001 DROP VIEW IF EXISTS `vendas_detalhadas`*/;
SET @saved_cs_client     = @@character_set_client;
/*!50503 SET character_set_client = utf8mb4 */;
/*!50001 CREATE VIEW `vendas_detalhadas` AS SELECT 
 1 AS `ID da Venda`,
...
--
-- Final view structure for view `vendas_detalhadas`
--
...
/*!50001 VIEW `vendas_detalhadas` AS select `v`.`id_vendas` AS `ID da Venda`,`c`.`nome_clientes` AS `Nome do Cliente`,`v`.`data` AS `Data da Venda`,`p`.`modelo` AS `Item Vendido`,`v`.`valor_total` AS `Valor Gasto`,`d`.`porcentagem` AS `Desconto`,`f`.`nome_funcionarios` AS `Vendedor` from ((((`vendas` `v` join `clientes` `c` on((`v`.`cliente` = `c`.`id_clientes`))) join `produtos` `p` on((`v`.`item_vendido` = `p`.`id_produtos`))) join `descontos` `d` on((`v`.`descontos` = `d`.`id_descontos`))) join `funcionarios` `f` on((`v`.`vendedor` = `f`.`id_funcionarios`))) */;
...
-- Dump completed on 2024-06-25  8:55:29
